...
def main():
    ...
    csim_cscore  = resultsim[0:1] # map(int, resultsim[0:1])
    ...
    total_score = int(csim_cscore[0]) + trans32_score + trans64_score + trans61_score
    # total_score = next(csim_cscore, None) + trans32_score + trans64_score + trans61_score

    # Summarize the results
    print ("\nCache Lab summary:")
    print ("%-22s%8s%10s%12s" % ("", "Points", "Max pts", "Misses"))
    print ("%-22s%8.1f%10d" % ("Csim correctness", int(csim_cscore[0]), 
                              maxscore['csim']))
